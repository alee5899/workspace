...
import study.StudentUtil;

public class myStudent implements StudentUtil {

  @Override
  public int getTotalScoreStudent(Student student) {
    return 0;
  ...
  public Student getHighScoreStudent(Student s1, Student s2) {
      return getTotalScoreStudent(s1) > getTotalScoreStudent(s2) ? s1 : s2;
    }
  ...
  public String getGradeByStudentName(Student[] students, String name){
    ...
    for (int i  = 0; i < students.length; i++) {
      if (students[i].getName().equals(name)) {
        double avg = getTotalScoreStudent(students[i]) / 3.0;
  ...
  public int[] getTotalScoresToArray(Student[] students) {
    ...
    for (int i = 0; i < students.length; i++){
      resultArr[i] = getTotalScoreStudent(students[i]);
    }
    return resultArr;
  }
}
