...
-- 단일행 함수가 아닌 함수의 예시
-- 이 함수들은 테이블에 저장된 데이터 수에 따라 조회  결과 객수가 달라진다 

SELECT comm, IFNULL(comm, 0), CONCAT(empno, '-',ename),SUBSTR(job,1,1)
FROM emp; 
...
-- 단일행 함수 사용 시 주의점 1
-- COUNT 와 AVG 함수 사용 시에는 NULL 데이터에 유의!
-- COUNT 함수에서 데이터 갯수는 NULL데이터는 죄외한 갯수를 알려줌
-- 결론1 : 만약, 테이블에 저장된 데이터의 갯수를 알고 싶다면 NULL데이터가
-- 들어간 컬럼으로 COUNT()함수를 사용하는것은 금지!
...
#부서별 커미션의 평균을 조회
#단, 10번부서는 제외하고,평균은 전체 사원수를 기준으로 조회한다.
...
#직급별 최소 급여를 조회하되, 사원 지급은 제외하며
#직급별 최소 급여가 300 미만인 데이터로 제외한다
#조회 결과 데이터를 직급 기준 오름차순으로 정렬하세여
SELECT JOB,MIN(SAL) 
FROM emp
WHERE JOB != '사원'
GROUP BY JOB
