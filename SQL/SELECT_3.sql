...
SELECT STU_NUM AS 학생번호,STU_NAME 학새명,KOR_SCORE,ENG_SCORE FROM study_student
WHERE KOR_SCORE >=60 AND ENG_SCORE >= 70; 

-- 2. 디자인반 혹은 영상반에 소속된 학생 중, 취미가 null이 아닌 학생들의 모든 컬럼 정보를 조회하시오.
SELECT * FROM study_student
WHERE hobby IS NOT NULL AND class_name IN('디자인반','영상반');

-- 3. 김씨 성을 지닌 학생 중, 국어점수가 70점에서 85점 사이인 학생들의 학번, 이름, 국어점수를 조회하되,
...
SELECT STU_NUM,STU_NAME,KOR_SCORE + ENG_SCORE AS 총합 FROM study_student
