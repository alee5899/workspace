...
-위와 같이 특정 범위를 조건으로 작성할 때는 BETWEEN 연산자 사용!
...
-- 위의 쿼리문은 아래처럼 IN연산자를 사용하여 작성할수도 있다.
...
-- 조회데이터 정렬하기
-- 정렬 문법 : ORDER BY 정렬기준컬럼 정렬방법;(오름차순,내릴차순);
-- 오름차순 : ASC,생략  내림차순 : DESC
-- 정렬을 위한 ORDER BY는 모조건 조회 쿼리문의 가장 마지막에 작성
...
-- 부서번호가 10번이 아닌 직원들의 
-- 사번,사원명,부서번호를 조회하되,
-- 부서번호 기준 오름차순 정렬후 사번기준 내림차순 정렬
SELECT EMPNO,ENAME,DEPTNO FROM emp
WHERE DEPTNO != 10
ORDER BY DEPTNO ASC,EMPNO DESC;

-- EMP테이블에서 직급이 사원 과장 부장인 사원들이
-- 사원명 직급 급여를 조회하되, 
-- 우선 직급기준 내림차순 정렬후, 급여기준 오름차순으로 정렬
...
-- 쿼리문 작성 시 BETWEEN연산자를 사용하여 작성하시오.
...
ORDER BY JOB ASC;
-- 3. 부서번호가 10, 20인 부서에 소속된 직원 중, 이름에 ‘
-- 이’가 포함된 직원의 사번, 사원명, 부서번호, 급여를 조회하되, 부서번호 기준 내림차순으로 정렬 후,
...
SELECT * FROM emp
WHERE SAL BETWEEN 400 AND 800 AND ENAME LIKE '%기' AND  COMM IS NULL; 
